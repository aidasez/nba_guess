...
def find_outcome():
    ...
    for result in range(0,len(response_result)):
        ...
        if overall_winner == "home":
            overall_winner = f"{home_score} + {game_match[0]["home_team"]}"
        else:
            overall_winner = f"{away_score} + {game_match[1]["away_team"]}"
...
def get_response_api():
    ...
    for i in range(len(response_moneyline)):
        ...
        # moneyline bookmaker (you can pick the first, or also paddypower)
